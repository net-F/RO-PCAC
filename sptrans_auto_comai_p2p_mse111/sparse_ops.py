...
import cuda_sparse_ops_hx
import cuda_sparse_ops_test


class AttenDotProduct(Function):
    @staticmethod
    def forward(ctx, query, key, out_F, kq_map):
        assert (query.is_contiguous() and key.is_contiguous() and out_F.is_contiguous())
        ctx.m = kq_map.shape[1]
        _, ctx.h, ctx.c = query.shape
        # ctx.kkk = key.shape[0]
        ctx.save_for_backward(query, key, kq_map)
        cuda_sparse_ops_hx.dot_product_forward(ctx.m, ctx.h, ctx.c, query, key,
                                               out_F, kq_map)
        return out_F

    @staticmethod
    def backward(ctx, grad_out_F):
        query, key, kq_map = ctx.saved_tensors
        grad_query = torch.zeros_like(query)
        grad_key = torch.zeros_like(key)

        cuda_sparse_ops_hx.dot_product_backward(ctx.m, ctx.h, ctx.c, query, key,
                                                kq_map, grad_query, grad_key, grad_out_F)
        return grad_query, grad_key, None, None


atten_dot_product_cuda = AttenDotProduct.apply


class AttenWeights_Transformer(Function):
    @staticmethod
    def forward(ctx, query, key, pos_enc, out_F, kq_map):
        ...
        cuda_sparse_ops_test.dot_product_forward(ctx.m, ctx.h, ctx.kkk, ctx.c, query, key, pos_enc,
                                                 out_F, kq_map)
        return out_F
    ...
    def backward(ctx, grad_out_F):
        # print('grad_out_F:')
        # print(torch.sum(grad_out_F.detach().cpu()))
        # print(grad_out_F.detach().cpu())
        # assert grad_out_F.is_contiguous()
        query, key, pos_enc, kq_map = ctx.saved_tensors
        grad_query = torch.zeros_like(query)
        grad_key = torch.zeros_like(key)
        grad_pos = torch.zeros_like(pos_enc)
        cuda_sparse_ops_test.dot_product_backward(ctx.m, ctx.h, ctx.kkk, ctx.c, query, key, pos_enc,
                                                  kq_map, grad_query, grad_key, grad_pos, grad_out_F)
        return grad_query, grad_key, grad_pos, None, None


atten_weights_transfor = AttenWeights_Transformer.apply


class OriDotProduct(Function):
    @staticmethod
...
ori_dot_product_cuda = OriDotProduct.apply
...
class ScalarAttention(Function):
    ...
    def forward(ctx, weight, value, out_F, kq_indices):
        ...
        cuda_sparse_ops_hx.scalar_attention_forward(ctx.m, ctx.h, ctx.c, weight, value, out_F,
                                                    kq_indices)
        return out_F
    ...
    def backward(ctx, grad_out_F):
        ...
        cuda_sparse_ops_hx.scalar_attention_backward(ctx.m, ctx.h, ctx.c, weight, value,
