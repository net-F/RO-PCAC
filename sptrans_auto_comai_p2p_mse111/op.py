...
def n_scale_batch(x_windows, margin=0.01):
    ...
    x_max, x_min, y_max, y_min, z_max, z_min = x.max(dim=1)[0], x.min(dim=1)[0], y.max(dim=1)[0], y.min(dim=1)[0], \
                                               z.max(dim=1)[0], z.min(dim=1)[0]
    x_max, x_min, y_max, y_min, z_max, z_min = x_max.unsqueeze(-1), x_min.unsqueeze(-1), y_max.unsqueeze(
        -1), y_min.unsqueeze(-1), z_max.unsqueeze(-1), z_min.unsqueeze(-1)
...
def get_likelihood(feature, mu, sigma):
    gaussian = torch.distributions.laplace.Laplace(mu, sigma)
    lower = gaussian.cdf(feature - 0.5)
    upper = gaussian.cdf(feature + 0.5)
