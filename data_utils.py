...
def read_h5_geo(filedir):
    pointcloud = h5py.File(filedir, 'r')
    for key in pointcloud.keys():
        print(pointcloud[key].name)
...
def read_h5_geo_color(filedir):
    pointcloud = h5py.File(filedir, 'r')
    # for key in pointcloud.keys():
    #     print(pointcloud[key].name)
    pc_coords = h5py.File(filedir, 'r')['coords'][:]
    pc_feats = h5py.File(filedir, 'r')['feats'][:]

    coords = pc_coords[:, 0:3].astype('int')
    feats = pc_feats[:, 0:3].astype('int')

    return coords, feats


def read_h5_scannet(filedir):
    pc = h5py.File(filedir, 'r')['data'][:]
    coords = pc[:, 0:3].astype('int16')
    attri = pc[:, 3:6].astype('float32')
...
def read_ply_ascii_scannet(filedir):
    files = open(filedir)
    data = []
    for i, line in enumerate(files):
        wordslist = line.split(' ')
...
def get_nearest_feats(x, downscaled_x, downscale_factor):
    batch_size = torch.max(x.coords[:, 0]).item() + 1
    feats_downscaledx_list = []
    for b_idx in range(0, batch_size):
        indices_x = (x.coords[:, 0] == b_idx)
        coordinates_x = x.coords[indices_x][:, 1:4].float()
        features_x = x.feats[indices_x]

        indices_downscaledx = (downscaled_x.coords[:, 0] == b_idx)
        coordinates_downscaledx = downscaled_x.coords[indices_downscaledx][:, 1:4].float()

        _, query_idx, _ = pytorch3d.ops.knn_points(coordinates_downscaledx.unsqueeze(0) * downscale_factor, coordinates_x.unsqueeze(0), K=1)
        feats_downscaledx = pytorch3d.ops.knn_gather(features_x.unsqueeze(0), query_idx).squeeze()
        feats_downscaledx_list.append(feats_downscaledx)

    features_downscaledx = torch.cat(feats_downscaledx_list)
    downscaled_x.feats = features_downscaledx
    return downscaled_x
...
def sp2ply(x, save_path):
    batch_size = torch.max(x.coords[:, 0]).item() + 1

    for b_idx in range(0, batch_size):
        indices_x = (x.coords[:, 0] == b_idx)
        coords = x.coords[indices_x][:, 1:4].detach().cpu().numpy().astype('int16')
        feats = yuv2rgb(x.feats[indices_x].detach().cpu().numpy().astype('float32') * 255)

        savefiledir = save_path + str(b_idx) + '.ply'

        if os.path.exists(savefiledir): os.system('rm ' + savefiledir)
        f = open(savefiledir, 'a+')
        f.writelines(['ply\n', 'format ascii 1.0\n'])
        f.write('element vertex ' + str(coords.shape[0]) + '\n')
        f.writelines(['property float x\n', 'property float y\n', 'property float z\n'])
